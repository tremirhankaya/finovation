...
/* ── Icons ── */
const TagIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: '#2ec4a7' }}>
...
/* ── Section Icons ── */
const DocIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: '#2ec4a7' }}>
...
export default function FundDesignApprovalPage() {
  ...
  const { init, error: initError } = useFundDraftInit({
    page: "APPROVAL",
    draftId,
  })
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setPositions(working.assets)
        setSourceRank(working.sourceRank ?? null)
        setFormError("")
      } catch (loadError) {
        if (controller.signal.aborted) return
        setFormError(
  ...
  const ruleRows: RuleRow[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        status: summary.maxSingleStock <= approvalInit.maxSingleStockMaxPct ? "ok" : "bad",
      },
      {
        key: "aboveThreshold",
        ...
        pct: (summary.aboveThresholdSum / approvalInit.aboveThresholdSumMax) * 100,
        status: summary.aboveThresholdSum <= approvalInit.aboveThresholdSumMax ? "ok" : "bad",
      },
      {
        key: "sector",
  ...
  const userRows: RuleRow[] = useMemo(() => {
    ...
    if (draft.tppMinPct != null || draft.tppMaxPct != null) {
      ...
      rows.push({
        key: "user_tpp",
        label: "Ozel TPP Orani",
        range: `%${min} - %${max}`,
        actual: `%${formatPct(summary.tppWeight)}`,
        pct: (summary.tppWeight / max) * 100,
        status: (summary.tppWeight >= min && summary.tppWeight <= max) ? "ok" : "warn",
      })
    }
    ...
    if (draft.minStockCount != null || draft.maxStockCount != null) {
      ...
      rows.push({
        key: "user_stockCount",
        label: "Ozel Hisse Sayisi",
        range: `${min} - ${max}`,
        actual: `${summary.stockCount}`,
        pct: (summary.stockCount / max) * 100,
        status: (summary.stockCount >= min && summary.stockCount <= max) ? "ok" : "warn",
      })
    }
  ...
  const forcedInfo = useMemo(() => {
    ...
    const { draft } = approvalInit
    if (!draft.forcedAssetCodes || draft.forcedAssetCodes.length === 0) return null
  ...
  const excludedInfo = useMemo(() => {
    if (!approvalInit) return null
    const { draft } = approvalInit
    if (!draft.excludedAssetCodes || draft.excludedAssetCodes.length === 0) return null
    const present = draft.excludedAssetCodes.filter(
      (code) => positions.some((p) => p.asset_code === code),
    )
    return {
  ...
  return (
    <FundDesignLayout step={6} wide>
      <section className={styles.page}>
        <header className={styles.header}>
          <h2 className={styles.pageTitle}>6. Onay ve Taslağı Kaydet</h2>
          <p className={styles.pageIntro}>
            Portföy tasarımınızı gözden geçirin ve kaydedin. Kaydettikten sonra
            taslak üzerinde değişiklik yapamazsınız.
          </p>
        </header>
        ...
        {isLoading || !approvalInit ? (
          <div className={styles.card}>
            <p className={styles.loadingBox}>Özet hazırlanıyor...</p>
          </div>
        ) : (
          <>
            ...
            <div className={styles.card}>
              <h3 className={styles.cardTitle}>
                <span className={styles.titleIcon}><DocIcon /></span>
                TASLAK ÖZETİ
              </h3>
              <div className={styles.summaryGrid}>
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><TagIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Fon Adı</span>
                    <span className={styles.summaryValue}>{approvalInit.draft.name ?? "—"}</span>
                  </div>
                </div>
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><DollarIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Fon Pay Fiyatı</span>
                    <span className={styles.summaryValue}>{formatMoney(approvalInit.draft.unitPrice)}</span>
                  </div>
                </div>
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><ShieldIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Yönetim Yaklaşımı</span>
                    <span className={styles.summaryValue}>{approachLabel(approvalInit.draft.managementApproach)}</span>
                  </div>
                </div>
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><LiquidityIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Hedef Likidite</span>
                ...
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><BarChartIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Min. Hisse Sayısı</span>
                    <span className={styles.summaryValue}>{approvalInit.draft.minStockCount ?? "—"}</span>
                  </div>
                </div>
                <div className={styles.summaryItem}>
                  <div className={styles.summaryIconBox}><StarIcon /></div>
                  <div className={styles.summaryTextBox}>
                    <span className={styles.summaryLabel}>Seçilen Portföy</span>
                    <span className={styles.summaryValue}>{proposalLabel(sourceRank)}</span>
                  </div>
                </div>
              </div>
            ...
            <div className={styles.card}>
              <h3 className={styles.cardTitle}>
                <span className={styles.titleIcon}><ShieldCheckIcon /></span>
                  ...
                  {ruleRows.map((row) => (
                    <tr key={row.key}>
                      <td className={styles.tdLabel}>
                        <span className={[styles.dot, row.status === "ok" ? styles.dotOk : styles.dotBad].join(" ")} />
                        {row.label}
                      </td>
                      ...
                      <td className={styles.tdValue}>
                        ...
                        <div className={styles.bar}>
                          <div
                            className={[styles.barFill, row.status === "ok" ? styles.barOk : styles.barBad].join(" ")}
                            style={{ width: `${Math.min(100, row.pct)}%` }}
                          />
                        </div>
                      </td>
                      <td className={styles.tdStatus}>
                        <span className={[styles.statusBadge, row.status === "ok" ? styles.badgeOk : styles.badgeBad].join(" ")}>
                          {row.status === "ok" ? "Uygun" : "Uygun Degil"}
                        </span>
                      </td>
                    </tr>
                  ))}
            ...
            {(userRows.length > 0 || forcedInfo || excludedInfo) && (
              <div className={styles.card}>
                <h3 className={styles.cardTitle}>
                  <span className={styles.titleIcon}><TargetIcon /></span>
                  ...
                  <table className={styles.ruleTable}>
                    ...
                    <tbody>
                      {userRows.map((row) => (
                        <tr key={row.key}>
                          <td className={styles.tdLabel}>
                            <span className={[styles.dot, row.status === "ok" ? styles.dotOk : styles.dotWarn].join(" ")} />
                            {row.label}
                          </td>
                          <td className={styles.tdRange}>{row.range}</td>
                          <td className={styles.tdValue}>
                            ...
                            <div className={styles.bar}>
                              <div
                                className={[styles.barFill, row.status === "ok" ? styles.barOk : styles.barWarn].join(" ")}
                                style={{ width: `${Math.min(100, row.pct)}%` }}
                              />
                            </div>
                          </td>
                          <td className={styles.tdStatus}>
                            <span className={[styles.statusBadge, row.status === "ok" ? styles.badgeOk : styles.badgeWarn].join(" ")}>
                              {row.status === "ok" ? "Uygun" : "Uygun Degil"}
                            </span>
                          </td>
                        </tr>
                ...
                {(forcedInfo || excludedInfo) && (
                  <div className={styles.assetCardsRow}>
                    {forcedInfo && (
                      <div className={[styles.assetCard, forcedInfo.missing.length > 0 ? styles.assetCardWarn : styles.assetCardOk].join(" ")}>
                        <div className={styles.assetCardHeader}>
                          <strong>Zorunlu Hisseler</strong>
                          <span className={styles.assetRatio}>{forcedInfo.inPortfolio}/{forcedInfo.total}</span>
                        </div>
                        <p className={styles.assetSub}>{forcedInfo.codes.join(", ")}</p>
                        <div className={styles.chipRow}>
                          {forcedInfo.codes.map((code) => {
                            const inPortfolio = !forcedInfo.missing.includes(code)
                            return (
                              <span key={code} className={[styles.chip, inPortfolio ? styles.chipOk : styles.chipWarn].join(" ")}>
                                {code}
                              </span>
                            )
                          })}
                        ...
                        {forcedInfo.missing.length > 0 && (
                          <p className={styles.assetNote}>
                            ⚠ {forcedInfo.missing.join(", ")} portföyden çıkarılmış.
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {excludedInfo && (
                      <div className={[styles.assetCard, excludedInfo.present.length > 0 ? styles.assetCardWarn : styles.assetCardOk].join(" ")}>
                        <div className={styles.assetCardHeader}>
                          <strong>Hariç Tutulan Hisseler</strong>
                          <span className={styles.assetRatio}>{excludedInfo.removedCount}/{excludedInfo.total}</span>
                        </div>
                        <p className={styles.assetSub}>{excludedInfo.codes.join(", ")}</p>
                        <div className={styles.chipRow}>
                          {excludedInfo.codes.map((code) => {
                            const removed = !excludedInfo.present.includes(code)
                            return (
                              <span key={code} className={[styles.chip, removed ? styles.chipOk : styles.chipWarn].join(" ")}>
                                {code}
                              </span>
                            )
                          })}
                        ...
                        {excludedInfo.present.length > 0 && (
                          <p className={styles.assetNote}>
                            ⚠ {excludedInfo.present.join(", ")} portföye geri eklenmiş.
                          </p>
                        )}
                      </div>
                    )}
            ...
            <div className={styles.confirmCard}>
              <div className={styles.confirmContent}>
                <p className={styles.confirmTitle}>Tüm bilgilerin doğruluğunu onaylıyorum.</p>
                <p className={styles.confirmSub}>
                  Onayladıktan sonra taslak kaydedilecek ve düzenleme yapılamayacaktır.
                </p>
            ...
            {hasViolation && (
              <p className={styles.blockedNote}>
                Kural ihlali olduğu için taslak kaydedilemez. Önceki adıma
                dönüp portföyü düzeltin.
              </p>
            )}
