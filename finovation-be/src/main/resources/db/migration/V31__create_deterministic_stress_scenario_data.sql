-- V27.1 creates these objects before the V28 and V29 seed migrations.
-- These guards make the original V31 step a safe no-op in the corrected order.

IF COL_LENGTH('dbo.stress_scenarios', 'horizon_days') IS NULL
BEGIN
    ALTER TABLE dbo.stress_scenarios
        ADD horizon_days SMALLINT NULL;
END;

IF OBJECT_ID('dbo.stress_scenario_asset_shocks', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('dbo.stress_scenario_asset_paths', 'U') IS NULL
BEGIN
